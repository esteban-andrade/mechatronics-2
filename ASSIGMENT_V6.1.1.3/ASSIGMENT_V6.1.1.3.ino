...
ISR(ADC_vect) {
  ...
  switch (ADMUX & 0x0F) { //Here we are only looking at the last for bits by setting the first for bits to 0 (just for the switch statement the bits in ADMUX don't actually change)
    ...
    case 0: //Don't need the C0 --> only 0 as we cleared the first 4 bits ^^
      //read_lcd_buttons();   //<-- this function is doing another conversion - this method is called as soon as a conversion finishes - that's the point of an interrupt (ISR). We already have the result
      lcd.setCursor(0, 0);
      lcd.print((result / 1000) % 10);           //This is simply a way to print a value to the display without it overlapping what is already there (ie 54 will print as 0054) 
      lcd.print((result / 100) % 10);
      lcd.print((result / 10) % 10);
      lcd.print((result / 1) % 10);
